...
def orchestrator_function(context: df.DurableOrchestrationContext):
    ...
    mapTasks = []
    for file in files:
        mapTasks.append(context.call_activity("MapperActivity", file))
    
    mapTasks = yield context.task_all(mapTasks)
    
    
    #Call the shuffler
    shuffleTask = yield context.call_activity('ShufflerActivity', mapTasks)

    
    #Call the reducers
    reduceTasks = []
    for file in shuffleTask:
        reduceTasks.append(context.call_activity("ReducerActivity", file))
